...
# Note: Test cases #2 and #3 fail

calc_sum <- function(n, multiple) {
  num_of_multiples <- (n - 1) %/% multiple
  multiple * (num_of_multiples * (num_of_multiples + 1)) %/%  2
}

f <- file("stdin")
open(f)
lineCount <- as.integer(readLines(f, n = 1))

for(i in seq_along(1:lineCount)) {
  ...
  write(sum_3 + sum_5 - sum_15, stdout())
